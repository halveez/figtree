...
solar_data <- read.csv(file = "data/solarday.csv", header = TRUE, sep = ",", fileEncoding = "UTF-8-BOM", stringsAsFactors = FALSE)
...
```{r overall_prep}
#We first combine all of the separate datasets by the Day column
overall <- merge(solar_data, figs_all, by.x = "Day", by.y = "Day")
overall <- merge(overall, daily_weather, by.x = "Day", by.y = "Day")
...
overall %>% ggplot(aes(x = Day)) + geom_point(aes(y = Combined, color = "green")) + geom_smooth(aes(y = Energy_Produced_Wh/1500, color = "red")) + geom_point(aes(y = Rain))
